...
theorem Loeb_without_GoedelIT2 {σ} : (⊢ₐ[T ∔ Γ] σ) ↔ (⊢ₐ[T ∔ Γ] Pr[T ∔ Γ](σ) ⇒ σ) := by
  apply Iff.intro;
  . intro H;
    exact ElimImply' ⟨K', H⟩;
  . intro H;
    ...
    have h₁  : ⊢ₐ[T ∔ Γ] K ⇒ (Pr[T ∔ Γ](K) ⇒ σ) := equiv_mp hK;
    have h₂  : ⊢ₐ[T ∔ Γ] Pr[T ∔ Γ](K ⇒ (Pr[T ∔ Γ](K) ⇒ σ)) := D1 h₁;
    have h₃  : ⊢ₐ[T ∔ Γ] (Pr[T ∔ Γ](K) ⇒ Pr[T ∔ Γ](Pr[T ∔ Γ](K) ⇒ σ)) := ElimImply' ⟨D2, h₂⟩;
    have h₄  : ⊢ₐ[T ∔ Γ] (Pr[T ∔ Γ](Pr[T ∔ Γ](K) ⇒ σ) ⇒ (Pr[T ∔ Γ](Pr[T ∔ Γ](K)) ⇒ Pr[T ∔ Γ](σ))) := D2;
    have h₅  : ⊢ₐ[T ∔ Γ] (Pr[T ∔ Γ](K) ⇒ Pr[T ∔ Γ](Pr[T ∔ Γ](K)) ⇒ Pr[T ∔ Γ](σ)) := imply_trans ⟨h₃, h₄⟩;
    have h₆  : ⊢ₐ[T ∔ Γ] (Pr[T ∔ Γ](K) ⇒ Pr[T ∔ Γ](Pr[T ∔ Γ](K))) := D3;
    have h₇  : ⊢ₐ[T ∔ Γ] (Pr[T ∔ Γ](K) ⇒ Pr[T ∔ Γ](σ)) := ElimImply' ⟨ElimImply' ⟨S', h₅⟩, h₆⟩;
    have h₈  : ⊢ₐ[T ∔ Γ] Pr[T ∔ Γ](K) ⇒ σ := imply_trans ⟨h₇, H⟩;
